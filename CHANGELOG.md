...
- Dependency submission, pinned signed-candidate provenance, product verification, and fresh exact-head checks after this documentation update remain pending. The local environment has no Gradle wrapper/system Gradle or Android SDK/ADB, so no local Android build or emulator run is claimed. No previous APK may be reused to fill this gap.
