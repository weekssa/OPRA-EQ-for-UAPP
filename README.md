...
The SIMGOT EW300 DSP cable is implemented on the v0.7 beta branch through the shared My DAC, finite-hardware, and Direct Flash framework. The exact five-band raw transport, strict session boundary, guarded Apply/Flash/Reset paths, and operation evidence are covered by automated code and protocol fixtures. The current candidate still requires one consolidated physical session before release or public support. The accepted Save qualification is historical evidence and is not repeated. See [EW300 status](docs/V0.7_EW300_DSP_STATUS.md), [validation ledger](docs/EW300_VALIDATION_LEDGER.md), and [protocol notes](docs/EW300_DSP_PROTOCOL_NOTES.md).
