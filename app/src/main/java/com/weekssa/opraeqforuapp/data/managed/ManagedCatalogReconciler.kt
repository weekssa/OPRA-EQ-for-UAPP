...
internal fun reconcileManagedProfiles(
    ...
    /** Legacy name retained through v0.3; this is the per-headphone new-EQ notification preference. */
    ...
    val reconciledCurrent = currentProfiles.map { profile ->
        ...
        if (existing == null) {
            ...
            ManagedProfileEntity(
                ...
                productId = productId,
                // Final v0.3 behavior is explicit selection only. Notifications create review
                // attention; they never silently add a newly discovered EQ to My EQs.
                selected = false,
            ...
            // Verification promotion and the notification preference never select an EQ. Preserve
            // only an explicit prior selection while the canonical source remains usable.
            val selected = selectedBeforeMigration && !becameUnusable
