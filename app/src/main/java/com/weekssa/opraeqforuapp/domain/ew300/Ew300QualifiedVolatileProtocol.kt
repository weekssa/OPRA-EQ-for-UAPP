...
/** Exact raw transport evidence from the owner's SIMGOT EW300 DSP cable. This is not a production DAC adapter: it does not decode acoustic values, infer a filter type, provide a persistence command, or register the cable as supported hardware. */
object Ew300QualifiedVolatileProtocol {
    ...
    fun qualifiedVolatileRegisterAddresses(): List<Int> = (FIRST_BAND_REGISTER..LAST_BAND_REGISTER).toList()
    fun isQualifiedVolatileRegister(address: Int): Boolean = address in FIRST_BAND_REGISTER..LAST_BAND_REGISTER
    fun readReport(address: Int): ByteArray {
        require(address in 0..0xFF) { "EW300 register address must fit one byte." }
        return byteArrayOf(REPORT_ID.byte(), address.byte(), 0, 0, 0, READ_COMMAND.byte(), 0, 0, 0, 0, 0)
    }
    fun temporaryWriteReport(address: Int, data: ByteArray): ByteArray {
        require(isQualifiedVolatileRegister(address)) { "Only physically qualified EW300 volatile band registers may be written." }
        require(data.size == 4) { "An EW300 band field must contain exactly four bytes." }
        return byteArrayOf(REPORT_ID.byte(), address.byte(), 0, 0, 0, WRITE_COMMAND.byte(), 0, data[0], data[1], data[2], data[3])
    }
