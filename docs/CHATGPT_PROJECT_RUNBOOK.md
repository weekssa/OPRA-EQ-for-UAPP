...
The accepted physical transaction evidence is E037-E040 on the exact authorized fingerprint `vid=31b2|pid=111|manufacturer=LE XIAN|product=SIMGOT EW300 DSP|serial=2024-07-03-0000-0000-0000|interface=3`, source `381286eb7ea29b0707e6da75f3565bada1d73928`: physical Apply, Flash, exact-baseline post-Save Restore, and Reset all reached verified outcomes with final readback. Do not repeat Flash, Restore, Apply, Reset, or the already accepted E001 Save qualification. Replay and competing-connection-job counters are null/unmeasured, not zero.

The last signed candidate before the current documentation reconciliation was source `3fbb9688d2774ca78b2c9dc943e00020046ed1f7`; Android CI #1711, CodeQL #1596, Catalog #2009, Priority community coverage #1494, Dependency Submission #2016, and Signed Candidate #1315 passed on that exact source. This documentation-only reconciliation creates a new source head, so no earlier candidate's checks or signature qualify the new head. The exact usable APK, checksum, signer, artifact digest, and all gate links must be refreshed in PR #23 for one identical source SHA before any owner check.

After that fresh candidate is ready, the only remaining owner-facing check is non-mutating: confirm the read-only exact-device report, save one Personal EQ capture from the observed five-band Peak state and verify its values/provenance, then open the My EQs Flash review and cancel before final confirmation. Do not execute Apply, Flash, Reset, Restore, Save qualification, or any other hardware write. If identity, state-known status, captured values/provenance, or UI gating is unclear, stop and report it; do not infer unsupported hardware.

Keep PR #23 draft and v0.7.0 NO-GO. Merge, publication, and any public EW300 support claim require explicit owner approval.
...
The exact USB identity and raw five-band transport are documented from the owner’s evidence. A reproducible same-earpiece acoustic cross-check qualifies direct-Hz Peak decoding; native non-Peak hardware labels remain gated. Register `0x66` is conservatively classified as ordinary digital DAC/playback gain, not a dedicated EQ preamp, so it is excluded from EQ identity and captured profiles. Do not infer compatibility from a chipset, browser tool, community report, or similarity to FiiO/JCALLY behavior. The current recovery implementation uses a strong EW300 string/interface/session fingerprint and keeps VID/PID-only discovery read-only. The shared EQ pipeline is source-neutral across OPRA, AutoEQ, community/general sources, imports, Personal EQs, and DAC captures. An Android-free declarative capability batch provides strict read-only preflight/reporting. The exact product candidate exposes only guarded Peak Apply/Flash/Reset behavior: automatic reconnect is blocked before the one Save send, mutation replay is prohibited, and readable/JSON operation evidence is available. Source low/high shelves are adapted only through a complete-response fit to bounded Peak-only EW300 output; this is an Optimized conversion path, not native shelf capture or hardware support. The EW300 Device tab exposes only read-only playback/global-gain state, active Peak count, connection freshness, and operation-report/reconnect status; it does not copy Black Pearl controls. The signed validation candidate also contains a separate exact-baseline post-Save restoration action that restores the captured five-band/global-gain state with one Save and byte-for-byte final readback; ordinary builds do not expose it. Current physical evidence is E037-E040: Apply, Flash, exact-baseline post-Save Restore, and Reset are verified on source `381286eb7ea29b0707e6da75f3565bada1d73928`; do not repeat those operations or Save qualification. The last signed candidate before documentation reconciliation is `3fbb9688d2774ca78b2c9dc943e00020046ed1f7` and passed its exact-head gates, but this documentation change requires fresh checks/signing. After a fresh same-head candidate is fully gated, the only owner check is a non-mutating Personal EQ capture and My EQs Flash-review/cancel check. Android permission for a re-enumerated instance remains OS-controlled. Merge, publication, and public support claims require explicit owner approval.
...
Physical transaction qualification is complete on source 381 in E037-E040. These results are not transferred to a later source as a new physical run, but the documentation/UI follow-up does not justify repeating any mutation. The only remaining owner-facing check, after the fresh candidate passes all gates, is read-only capability confirmation, one Personal EQ capture with values/provenance review, and opening then canceling the My EQs Flash review before the final write confirmation. No Apply, Flash, Reset, Restore, or Save is requested.
