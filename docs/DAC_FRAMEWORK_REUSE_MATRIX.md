...
| Mutation safety | Shared strict baseline/restore and fail-closed result types | EW300 one-Save commit and pre-Save volatile readback | Current code and flasher tests; current-head physical result pending. |
| Re-enumeration | Shared generation/detach observation | Save releases replacement reconnect only after accepted commit | New gate plus trace; physical replacement session pending. |
