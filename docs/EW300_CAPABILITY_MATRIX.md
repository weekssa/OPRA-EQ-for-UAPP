...
| Peak Apply / Flash / Reset transaction | SUPPORTED_AND_IMPLEMENTED | Current guarded flasher, shared session gate, pre-Save volatile readback, one Save, final readback, and replacement-session checks. E027 is a complete `APPLY` report with final readback match on candidate `676fadf`; E031 is the corresponding `FLASH` report on `dbc86b6`, showing 11 writes, one Save, volatile readback match, then coroutine cancellation during the disconnect boundary before replacement-session handling and final readback. E032 is the signed lifecycle-fix candidate, ready for one bounded physical Flash retest. |
