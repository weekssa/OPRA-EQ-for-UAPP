...
| Peak Apply / Flash / Reset transaction | SUPPORTED_AND_IMPLEMENTED | Guarded flasher, shared session gate, pre-Save volatile readback, one Save, final readback, exact replacement-session checks, and the new exact-baseline post-Save restoration transaction are implemented. E027 is a complete physical `APPLY` report only. E031/E033 are physical `FLASH` failures after Save/volatile match. The signed validation action restores the captured five-band/global-gain baseline with one Save and byte-for-byte final readback; ordinary builds do not expose it. **Physical Apply, Flash, exact restoration, and Reset are verified on source 381 (E037-E040); the latest verified signed head before this documentation-only synchronization is 2f99; this change itself is documentation-only.** |
...
| Exact post-Save baseline restoration | SUPPORTED_AND_IMPLEMENTED | Current software writes the captured qualified five-band pairs plus `0x66`, performs one guarded restoration Save, requires an exact fingerprint and strictly newer replacement generation after detach, and verifies complete byte-for-byte final readback. Physical Flash/restoration are verified on source 381 (E037-E038); the latest verified signed head before this documentation-only synchronization is 2f99; this change itself requires fresh gates and release remains NO-GO. |
