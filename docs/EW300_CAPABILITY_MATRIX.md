...
| Peak Apply / Flash / Reset transaction | SUPPORTED_AND_IMPLEMENTED | Current guarded flasher, shared session gate, pre-Save volatile readback, one Save, final readback, and replacement-session checks. E027 is a complete `APPLY` report with final readback match on candidate `676fadf`; E030 records an unverified physical `FLASH` stop on `dbc86b6` followed by a read-only PASS, but no Flash operation JSON is attached. |
