...
| Exact post-Save baseline restoration | SUPPORTED_AND_IMPLEMENTED | Current software writes the captured qualified five-band pairs plus `0x66`, performs one guarded restoration Save, requires an exact fingerprint and strictly newer replacement generation after detach, and verifies complete byte-for-byte final readback. Physical Flash/restoration are verified on source 381 (E037-E038); current head e9 is documentation-only and release remains NO-GO. |
