...
3. Tap **Request read-only descriptor capture**, approve Android USB permission, and send the full report screenshot. The follow-up may briefly detach Android's media-button driver from HID interface 3 only; reconnect the cable immediately after the report appears. Stop if the app reports an error.
