...
- EW300 exposes all five native bands as guarded device state. Global gain becomes a first-class control only after the exact-device qualification gate passes.
...
- Reset EQ to flat only if the exact persistence/reset transaction passes the final physical gate;
...
8. If the persistence/reset gate is enabled in the exact candidate, reset EQ to flat and confirm verified flat readback; otherwise confirm Reset is absent and accurately labeled pending.
